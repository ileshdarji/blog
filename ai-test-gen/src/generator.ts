...
import type { CodebaseProfile, GenerateOptions, GeneratedFile } from './types.js';
...
function buildSystemPrompt(profile: CodebaseProfile): string {
  ...
  if (profile.exampleTests.length > 0) {
    ...
    parts.push('Use standard Playwright best practices: TypeScript, async/await, proper expect() assertions, and semantic or data-testid locators.');
  }
...
function buildUserPrompt(options: GenerateOptions, profile: CodebaseProfile): string {
  ...
  lines.push(`4. (Optional) Any additional case that a senior QA engineer would flag as high risk`);

  lines.push(`\n**Import path for test/expect:** \`${profile.testImportPath}\``);

  if (profile.selectorStrategy !== 'unknown') {
    lines.push(`**Selector strategy detected in this codebase:** ${profile.selectorStrategy}`);
  }
...
function conventionRules(profile: CodebaseProfile): string {
  ...
  if (profile.hasPageObjects) {
    rules.push('- This codebase uses Page Object classes — reference them in tests where appropriate');
  }
  ...
  rules.push('- Handle async operations with proper Playwright waiting — no `page.waitForTimeout()`');
...
function featureToFilename(feature: string): string {
  const slug = feature
    .toLowerCase()
    .replace(/[^a-z0-9\s-]/g, '')
    .trim()
