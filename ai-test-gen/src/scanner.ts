...
export async function scanTestSuite(testDir: string): Promise<CodebaseProfile> {
  ...
  const [specFiles, fixtureFiles, pageObjectFiles] = await Promise.all([
    glob(`${absDir}/**/*.spec.ts`, { ignore: '**/node_modules/**' }),
    glob(`${absDir}/**/fixture?(s).ts`, { ignore: '**/node_modules/**' }),
    glob(`${absDir}/**/pages/**/*.ts`, { ignore: '**/node_modules/**' }),
  ]);
  ...
  const examples = specFiles
    ...
    .map(f => ({
      filename: path.relative(absDir, f),
...
function extractCommonImports(
  ...
  // Intersect: keep imports that appear in ALL example files
...
function detectTestImportPath(content: string, hasFixtures: boolean): string {
  // If there are custom fixtures, tests likely import from a local path
  if (hasFixtures) {
